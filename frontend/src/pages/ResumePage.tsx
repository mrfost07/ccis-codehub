    ...
    <div className="min-h-screen bg-neutral-950">
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-neutral-400">Pick a template — your profile data fills in automatically</p>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mb-10">
          {TEMPLATES.map(t => (
            <button
              ...
              className={`group relative rounded-2xl overflow-hidden border-2 transition-all hover:scale-105 ${template === t.id ? 'border-purple-500' : 'border-neutral-700 hover:border-neutral-500'}`}
            >
              ...
              <div className="bg-neutral-900 px-3 py-2 text-left">
        ...
        <p className="text-center text-neutral-500 text-sm">Click any template to start building</p>
      </div>
    </div>
  )

  return (
    <div className="min-h-screen bg-neutral-950 print:bg-white">
      {/* Toolbar — hidden on print */}
      <div className="print:hidden">
        <Navbar />
        <div className="bg-neutral-900 border-b border-neutral-800 px-4 py-3 flex items-center gap-4 flex-wrap">
          <a href="/profile" className="flex items-center gap-1.5 text-neutral-400 hover:text-white transition text-sm">
            <ChevronLeft className="w-4 h-4" /> Back to Profile
          </a>
          <button onClick={() => setStep('pick')} className="flex items-center gap-1.5 text-neutral-400 hover:text-white transition text-sm">
            <ChevronLeft className="w-4 h-4" /> Templates
          </button>
          <div className="flex gap-2">
            {TEMPLATES.map(t => (
              <button key={t.id} onClick={() => setTemplate(t.id)}
                className={`px-3 py-1 rounded-full text-xs font-medium transition ${template === t.id ? 'bg-purple-600 text-white' : 'bg-neutral-800 text-neutral-400 hover:bg-neutral-700'}`}>
                {t.label}
              </button>
            ))}
          </div>
      ...
      <div className="print:hidden flex gap-0 h-[calc(100vh-113px)] overflow-hidden">
        {/* Left: Editor */}
        <div className="w-80 flex-shrink-0 bg-neutral-900 border-r border-neutral-800 overflow-y-auto p-4 space-y-4">
          <ResumeEditor data={data} setData={setData} />
        </div>
        {/* Right: Live Preview */}
        <div className="flex-1 overflow-y-auto bg-neutral-950 flex justify-center py-8 px-4">
    ...
    <div>
      ...
      {multiline
        ? <textarea value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} rows={3}
            className="w-full px-2 py-1.5 bg-neutral-800 border border-neutral-700 rounded text-sm text-white placeholder-neutral-600 focus:outline-none focus:border-purple-500 resize-none" />
        : <input value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
            className="w-full px-2 py-1.5 bg-neutral-800 border border-neutral-700 rounded text-sm text-white placeholder-neutral-600 focus:outline-none focus:border-purple-500" />
      }
    </div>
  )
}
...
function Section({ title, children }: any) {
  return (
    <div className="bg-neutral-800/40 rounded-xl p-3 space-y-2">
    ...
    <>
      <Section title="Personal Info">
        ...
        <div className="flex items-center justify-between py-1">
          <span className="text-xs text-neutral-400">Show profile photo</span>
          <button
            onClick={() => setData((d: ResumeData) => ({ ...d, showPhoto: !d.showPhoto }))}
            className={`relative w-10 h-5 rounded-full transition-colors ${data.showPhoto ? 'bg-purple-600' : 'bg-neutral-700'}`}
          >
      ...
      <Section title="Education">
        {data.education.map((ed, i) => (
          <div key={i} className="space-y-1.5 pb-2 border-b border-neutral-700 last:border-0">
        ...
        {data.experience.map((ex, i) => (
          <div key={i} className="space-y-1.5 pb-2 border-b border-neutral-700 last:border-0">
            <div className="flex justify-between items-start">
              <p className="text-xs text-neutral-400">{ex.company || 'New Entry'}</p>
        ...
        {data.projects.map((pr, i) => (
          <div key={i} className="space-y-1.5 pb-2 border-b border-neutral-700 last:border-0">
            <div className="flex justify-between">
              <p className="text-xs text-neutral-400">{pr.name || 'New Project'}</p>
  ...
  return (
    <div className="flex gap-1">
      <input value={val} onChange={e => setVal(e.target.value)} placeholder="Add skill..."
        ...
        className="flex-1 px-2 py-1 bg-neutral-800 border border-neutral-700 rounded text-xs text-white placeholder-neutral-600 focus:outline-none focus:border-purple-500" />
