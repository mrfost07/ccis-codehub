...
export default function Organizations() {
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      case 'program': return 'bg-purple-500/20 text-purple-400'
      case 'club': return 'bg-purple-500/20 text-purple-400'
      case 'interest': return 'bg-green-500/20 text-green-400'
      case 'official': return 'bg-amber-500/20 text-amber-400'
      default: return 'bg-neutral-500/20 text-neutral-400'
    }
  }

  const OrganizationCard = ({ org, showActions = true }: { org: Organization; showActions?: boolean }) => (
    <div className="bg-neutral-800/50 rounded-xl border border-neutral-700/50 overflow-hidden hover:border-purple-500/30 transition">
      {/* Cover */}
      <div className="h-24 bg-gradient-to-r from-purple-600/30 to-purple-600/30 relative">
        {org.cover_image_url && (
          <img src={org.cover_image_url} alt="" className="w-full h-full object-cover" />
        )}
        <div className="absolute -bottom-6 left-4">
          <div className="w-12 h-12 bg-neutral-700 rounded-xl flex items-center justify-center text-2xl border-2 border-neutral-800">
            {org.icon}
          </div>
        </div>
        {org.is_official && (
          <div className="absolute top-2 right-2 bg-amber-500/20 px-2 py-1 rounded-full flex items-center gap-1">
        ...
        <div className="flex items-start justify-between mb-2">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              {org.program && (
                <span className="text-xs text-neutral-400">{org.program}</span>
              )}
            </div>
          </div>
        </div>
              ...
              {org.is_member ? (
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="px-3 py-1 text-sm bg-neutral-700 hover:bg-red-600 text-neutral-300 hover:text-white rounded-lg transition"
                  >
                    Leave
                  </button>
                </div>
              ) : org.membership_status === 'pending' ? (
                <span className="px-3 py-1 text-sm bg-amber-500/20 text-amber-400 rounded-lg flex items-center gap-1">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-neutral-400">Discover and join communities</p>
          </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-6">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 bg-neutral-800 border border-neutral-700 rounded-lg text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                />
              </div>
              <div className="flex gap-2">
                <select
                  ...
                  className="px-4 py-2 bg-neutral-800 border border-neutral-700 rounded-lg text-white focus:ring-2 focus:ring-purple-500"
                >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {myOrgs.length > 0 ? (
              ...
              <div className="col-span-full text-center py-12">
                ...
                <p className="text-neutral-400 mb-4">Explore and join organizations to connect with others</p>
                <button
                  onClick={() => setActiveTab('discover')}
        ...
        {activeTab === 'invitations' && (
          <div className="space-y-4">
            {invitations.length > 0 ? (
              invitations.map(inv => (
                <div key={inv.id} className="bg-neutral-800/50 rounded-xl border border-neutral-700/50 p-4">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-neutral-700 rounded-xl flex items-center justify-center text-2xl">
                      ...
                      <div>
                        ...
                        {inv.message && (
                          <p className="text-neutral-300 text-sm mt-2">"{inv.message}"</p>
                        )}
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
              ...
              <div className="text-center py-12">
                <UserPlus className="w-16 h-16 text-neutral-600 mx-auto mb-4" />
                <h3 className="text-xl text-white mb-2">No pending invitations</h3>
                <p className="text-neutral-400">You'll see invitations here when someone invites you</p>
              </div>
            )}
          </div>
        )}
