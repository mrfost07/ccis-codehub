    ...
    if (!data) {
        return (
            <div className="min-h-screen bg-neutral-950 flex flex-col items-center justify-center text-white">
                ...
                <p className="text-neutral-400 mt-2">This quiz hasn't been taken yet.</p>
                <button onClick={() => navigate(-1)} className="mt-6 px-4 py-2 bg-neutral-800 rounded-lg text-sm hover:bg-neutral-700 transition">
                    Go Back
                </button>
            </div>
        );
        ...
        <div className="min-h-screen bg-neutral-950">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-3 text-sm text-neutral-400 mt-0.5">
                                ...
                                <span>•</span>
                                <span className={quiz.is_open ? 'text-green-400' : 'text-neutral-500'}>{quiz.status_text}</span>
                            </div>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <BarChart3 className="w-5 h-5 text-purple-400" />
                        <span className="text-sm font-medium text-neutral-300">Analytics</span>
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-6xl mx-auto px-4 py-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                    <StatCard icon={<Users className="w-5 h-5 text-purple-400" />} label="Participants" value={stats.total_participants} />
                    <StatCard icon={<Trophy className="w-5 h-5 text-amber-400" />} label="Avg Score" value={stats.average_score} />
                    <StatCard icon={<Target className="w-5 h-5 text-green-400" />} label="Avg Accuracy" value={`${stats.average_accuracy}%`} />
                    <StatCard icon={<CheckCircle className="w-5 h-5 text-green-400" />} label="Completion" value={`${stats.completion_rate}%`} />
                </div>
                        ...
                        {stats.hardest_question && (
                            <div className="bg-red-500/5 border border-red-500/20 rounded-xl p-4 flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-neutral-300 mt-1">{stats.hardest_question}</p>
                                </div>
                            </div>
                        )}
                        {stats.easiest_question && (
                            <div className="bg-green-500/5 border border-green-500/20 rounded-xl p-4 flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-neutral-300 mt-1">{stats.easiest_question}</p>
                                </div>
                            </div>
                        )}
                    </div>
                ...
                {/* Tabs */}
                <div className="flex gap-1 bg-neutral-900 p-1 rounded-xl mb-6 w-fit">
                ...
                {activeTab === 'overview' && (
                    <div className="space-y-4">
                        ...
                        {question_analytics.map(q => (
                            <div key={q.question_id} className="bg-neutral-900/50 border border-neutral-800 rounded-xl p-4">
                                <div className="flex items-center justify-between mb-3">
                                    <span className="text-sm text-neutral-400">Q{q.order}</span>
                        ...
                        {question_analytics.length === 0 && (
                            <p className="text-neutral-500 text-center py-8">No question data available yet.</p>
                        )}
                    </div>
                )}
                ...
                {activeTab === 'leaderboard' && (
                    <div className="bg-neutral-900/50 border border-neutral-800 rounded-xl overflow-hidden">
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="border-b border-neutral-800 text-left">
                                        <th className="px-4 py-3 text-xs text-neutral-500 font-medium uppercase tracking-wider">#</th>
                                        <th className="px-4 py-3 text-xs text-neutral-500 font-medium uppercase tracking-wider">Student</th>
                                        <th className="px-4 py-3 text-xs text-neutral-500 font-medium uppercase tracking-wider text-right">Score</th>
                                        <th className="px-4 py-3 text-xs text-neutral-500 font-medium uppercase tracking-wider text-right">Accuracy</th>
                                        <th className="px-4 py-3 text-xs text-neutral-500 font-medium uppercase tracking-wider text-right">Avg Time</th>
                                        <th className="px-4 py-3 text-xs text-neutral-500 font-medium uppercase tracking-wider text-right">Violations</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {leaderboard.map((entry) => (
                                        <tr key={entry.participant_id} className={`border-b border-neutral-800/50 hover:bg-neutral-800/30 transition ${entry.is_flagged ? 'bg-red-500/5' : ''}`}>
                                            <td className="px-4 py-3">
                                                {entry.rank <= 3 ? (
                                                    <span className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${entry.rank === 1 ? 'bg-amber-500/20 text-amber-400' :
                                                    ...
                                                    <span className="text-neutral-500 text-sm pl-2">{entry.rank}</span>
                                                )}
                                            </td>
                                            <td className="px-4 py-3">
                                                <div className="text-white text-sm font-medium">{entry.student_name}</div>
                                                <div className="text-neutral-500 text-xs">{entry.nickname}</div>
                                            </td>
                                            ...
                                            <td className="px-4 py-3 text-right text-neutral-400 text-sm">{entry.average_response_time}s</td>
                                            <td className="px-4 py-3 text-right">
                                                {entry.violations > 0 ? (
                        ...
                        {leaderboard.length === 0 && (
                            <p className="text-neutral-500 text-center py-8">No participants yet.</p>
                        )}
                    </div>
                )}
                ...
                {activeTab === 'questions' && (
                    <div className="space-y-3">
                        {question_analytics.map(q => (
                            <div key={q.question_id} className="bg-neutral-900/50 border border-neutral-800 rounded-xl overflow-hidden">
                                <button
                                    onClick={() => setExpandedQuestion(expandedQuestion === q.question_id ? null : q.question_id)}
                                    className="w-full px-5 py-4 flex items-center justify-between hover:bg-neutral-800/30 transition"
                                >
                                    <div className="flex items-center gap-4">
                                ...
                                {expandedQuestion === q.question_id && (
                                    <div className="px-5 pb-4 pt-2 border-t border-neutral-800 space-y-4">
                                        {/* Stats Row */}
                                        ...
                                        {Object.keys(q.answer_distribution).length > 0 && (
                                            <div>
                                                <p className="text-xs text-neutral-500 uppercase tracking-wider mb-2">Answer Distribution</p>
                                                        ...
                                                        .map(([key, count]) => {
                                                            ...
                                                            return (
                                                                <div key={key} className="flex items-center gap-3">
                                                                    <span className={`w-7 text-xs font-bold ${isCorrect ? 'text-green-400' : 'text-neutral-400'}`}>
                                                                        {key}
                                                                    </span>
                                                                    <div className="flex-1 bg-neutral-800 h-5 rounded overflow-hidden">
                                                                        <div
                                                                            className={`h-full rounded ${isCorrect ? 'bg-green-600/60' : 'bg-neutral-600/60'} transition-all`}
                                                                            style={{ width: `${pct}%` }}
                                                                        />
                                                                    </div>
