...
function CertificateCardPro({ cert, userName }: { cert: Certificate; userName: string }) {
  ...
  const COLORS = ['border-purple-500/30 bg-purple-500/5', 'border-blue-500/30 bg-blue-500/5', 'border-emerald-500/30 bg-emerald-500/5', 'border-orange-500/30 bg-orange-500/5']
  const ICON_COLORS = ['text-purple-400', 'text-blue-400', 'text-emerald-400', 'text-orange-400']
  const colorIdx = cert.career_path.name.charCodeAt(0) % COLORS.length

  return (
    <div className="group relative rounded-2xl overflow-hidden border border-slate-700/60 hover:border-purple-500/50 transition-all hover:shadow-xl hover:shadow-purple-500/10 bg-slate-900">
      ...
      <div className={`relative h-28 border-b ${COLORS[colorIdx]} flex flex-col items-center justify-center p-4 overflow-hidden`}>
        ...
        <p className="text-slate-500 text-[10px] mt-1 relative z-10">CCIS-CodeHub</p>
      </div>
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2 mb-3">
          <div>
            ...
            <p className="text-xs text-slate-500 font-mono">{cert.certificate_id}</p>
          </div>
          <div className="flex items-center gap-1 bg-emerald-500/10 border border-emerald-500/30 rounded-full px-2 py-0.5 flex-shrink-0">
...
function ClaimableCard({ item, onClaim, awarding }: { item: EligibilityItem; onClaim: (id: string) => void; awarding: string | null }) {
  return (
    <div className="relative rounded-2xl overflow-hidden border border-yellow-500/20 bg-slate-900/80">
      ...
      <div className="p-5">
        <div className="flex items-start justify-between mb-3">
          <div>
            ...
            <p className="text-xs text-yellow-400">Path completed — claim your certificate!</p>
          </div>
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-yellow-500 hover:bg-yellow-400 text-slate-900 text-sm font-bold rounded-xl transition disabled:opacity-60"
        >
    ...
    <div className="rounded-2xl border border-slate-700/50 bg-slate-900/60 p-5">
      <div className="flex items-start justify-between mb-3">
        <div>
          ...
          <p className="text-xs text-slate-500">{item.completed_modules} / {item.total_modules} modules</p>
        </div>
        ...
        <div className="mb-10">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-slate-400 text-sm">Complete learning paths to earn verified certificates</p>
            </div>
          </div>
            ...
            <div className="flex flex-wrap gap-4 mt-5">
              ...
              {claimable.length > 0 && (
                <div className="flex items-center gap-2 px-4 py-2 bg-yellow-500/10 border border-yellow-500/20 rounded-xl">
                  ...
                  <span className="text-slate-400 text-sm">Ready to Claim</span>
                </div>
              )}
              {inProgress.length > 0 && (
          ...
          <div className="space-y-10">
            ...
            {certificates.length === 0 && claimable.length === 0 && inProgress.length === 0 && (
              <div className="text-center py-20">
                ...
                <p className="text-slate-400 mb-6">Enroll in a learning path and complete all its modules to earn a certificate.</p>
