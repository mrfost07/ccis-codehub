...
function InstructorStudentsSection({ onViewUser }: InstructorStudentsSectionProps) {
    ...
    const UserAvatar = ({ user, size = 'md', gradient = 'from-purple-500 to-purple-500' }: {
        user: User,
        size?: 'sm' | 'md' | 'lg',
        gradient?: string
    }) => {
    ...
    if (loading) {
        return (
            <div className="space-y-4">
                {[...Array(3)].map((_, i) => (
                    <div key={i} className="bg-neutral-800/50 backdrop-blur-xl border border-neutral-700/30 rounded-2xl p-4 animate-pulse">
                        <div className="h-6 bg-neutral-700 rounded w-1/3 mb-3"></div>
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Filters */}
            <div className="bg-neutral-800/50 backdrop-blur-xl border border-neutral-700/30 rounded-2xl p-4 shadow-lg">
                ...
                <div className="space-y-3">
                    {filteredInstructors.length === 0 ? (
                        ...
                        filteredInstructors.map((instructor) => (
                            <div
                                key={instructor.id}
                                className="bg-neutral-800/50 backdrop-blur-xl border border-neutral-700/30 rounded-2xl shadow-lg overflow-hidden"
                            >
                                {/* Instructor Header */}
                                <div
                                    className="p-4 flex items-center justify-between cursor-pointer hover:bg-neutral-700/30 transition-colors"
                                ...
                                >
                                    ...
                                    >
                                        <UserAvatar user={instructor} size="md" gradient="from-purple-500 to-purple-500" />
                                        <div>
                                            ...
                                            <p className="text-neutral-400 text-sm">{instructor.email}</p>
                                        </div>
                                ...
                                {expandedInstructor === instructor.id && (
                                    <div className="border-t border-neutral-700/50 bg-neutral-900/30 p-4">
                                        ...
                                        <div className="space-y-2 max-h-64 overflow-y-auto">
                                            {students.slice(0, 10).map((student) => (
                                                <div
                                                    key={student.id}
                                                    className="flex items-center justify-between p-2 rounded-lg hover:bg-neutral-700/30 transition-colors cursor-pointer"
                                                    onClick={() => handleViewUser(student)}
                                                >
                                                    <div className="flex items-center gap-3">
                                                        <UserAvatar user={student} size="sm" gradient="from-green-500 to-purple-500" />
                                                        <div>
                                                            ...
                                                            <p className="text-neutral-400 text-xs">{student.program} • Year {student.year_level}</p>
                                                        </div>
                                                    </div>
                                                </div>
                                            ))}
            ...
            {/* Students List */}
            {viewMode === 'students' && (
                <div className="bg-neutral-800/50 backdrop-blur-xl border border-neutral-700/30 rounded-2xl shadow-lg overflow-hidden">
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            <thead className="bg-neutral-700/50">
                                <tr>
                                    <th className="text-left px-4 py-3 text-neutral-300 font-medium text-sm">Student</th>
                                    <th className="text-left px-4 py-3 text-neutral-300 font-medium text-sm hidden sm:table-cell">Program</th>
                                    <th className="text-left px-4 py-3 text-neutral-300 font-medium text-sm hidden md:table-cell">Year</th>
                                    <th className="text-left px-4 py-3 text-neutral-300 font-medium text-sm hidden lg:table-cell">Joined</th>
                                </tr>
                            </thead>
                                            ...
                                            <td className="px-4 py-3">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-neutral-400 text-xs">{student.email}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-4 py-3 text-neutral-300 hidden sm:table-cell">{student.program || 'N/A'}</td>
                                            <td className="px-4 py-3 text-neutral-300 hidden md:table-cell">Year {student.year_level || 'N/A'}</td>
